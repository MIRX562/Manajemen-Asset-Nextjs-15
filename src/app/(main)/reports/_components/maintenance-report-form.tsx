...
import { maintenanceReport } from "@/actions/report-actions";
...
const items = [
  ...
  { id: "mechanic_id", label: "Mechanic ID" },
...
export default function MaintenanceReportForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      toast.promise(
        (async () => {
          const buffer = await maintenanceReport(values);
          exportFile(buffer, "maintenance_report", values.type);
        })(),
        {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  Choose maintenance status to export
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
